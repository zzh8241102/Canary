...
article_content_parser.add_argument(
    'article_id', type=str, required=True, help='article_id is required')
...
class ArticleContentApi(Resource):
    ...
    def get(self):
        ...
        if(Article.find_by_article_id(data['article_id'])):
            ...
            self.response_obj['data']['article']['article_tags'] = find_article_tags(
                data['article_id'])
            self.response_obj['data']['article']['article_comments'] = []
            self.response_obj['data']['article']['article_likes'] = fetch_article_like_num(
                data['article_id'])
